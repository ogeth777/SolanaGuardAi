...
export function generateRiskSummary(report: SecurityReport): string {
  ...
  if (report.marketData) {
     ...
     if (name.includes("(Sollet)") || name.includes("Wrapped Ethereum (Sollet)") || name.includes("soETH")) {
         risks.unshift("🚨 **CRITICAL: Deprecated Asset (Sollet).** This token is a legacy wrapped asset from the deprecated Sollet bridge. It is likely de-pegged and trading significantly below the real asset price.");
     }
     
     // Liquidity Analysis
     if (liquidityUsd < 1000) risks.push("🚨 **CRITICAL: Liquidity is negligible (< $1,000).** High risk of rugpull or inability to sell.");
     else if (liquidityUsd < 10000) risks.push("Liquidity is low (< $10k). Price impact on sells will be high.");
     else if (liquidityUsd > 100000) pros.push("💧 **Deep Liquidity:** > $100k liquidity ensures smoother trading.");
  ...
  let summary = `### 🤖 AI Security Verdict: **${report.verdict}**\n\n`;
